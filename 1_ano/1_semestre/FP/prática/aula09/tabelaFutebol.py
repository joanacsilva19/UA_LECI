...
#definir função lambda para retornar a diferença entre GM e GS
diferencaGMeGS = lambda reg: reg[GM] - reg[GS]
...
def main():
    ...
    tab = sorted(tabela, key=diferencaGMeGS, reverse= True)
    printTabela(tab)
    ...
    tab = sorted(tabela, key= lambda x: (pontos(x),diferencaGMeGS(x)), reverse= True)
    printTabela(tab)
